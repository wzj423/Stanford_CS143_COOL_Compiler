...
/*
  returns a pair<lowest_label,highest_label>
*/
inline std::pair<int,int> alloc_n_label(int n) {
  label_count+=n;
  return std::make_pair(label_count-n,label_count-1);
}
...
/**
 * debug emit functions
 * 
*/
void emit_dbg_out_int_obj(char *source,ostream &s) {
  if(strcmp(ACC,source)==0) {
    emit_push(ACC,s);
    emit_jal("IO.out_int",s);   
  } else {
    emit_push(ACC, s);
    emit_move(ACC,source,s);
    emit_push(ACC,s);
    emit_jal("IO.out_int",s);   
    emit_addiu(SP, SP, 4, s);
    emit_load(ACC, 0, SP, s);
...
void emit_dbg_out_int_value(char *source,ostream &s) {
  if(strcmp(ACC,source)==0) {
  ...
  for(auto c:classes_list) {
    ...
    for(auto attr:c->get_full_attrs()) {
      ...
      if(type==Bool) {
        ...
        /* REMEMBER: DO NOT use lookup(0) as this will take the int_const with index 0 (int_const_0) as the result!!! */
        str<<endl;        
      } else if(type==Str) {
        str<<WORD;
...
void typcase_class::code(ostream &s) {
  dbgStream<<"Coding typcase_class"<<endl;
  this->expr->code(s);
  emit_push(ACC,s);
  ...
  emit_bne(T1,T2,tag_valid_label,s);
  emit_jal("_case_abort",s);

  //not reaching top of the inheritance tree, 
  //iterating through branches to check the current class_tag
  emit_label_def(tag_valid_label,s);

  //coding each branch
  std::vector<int> branch_labels;
  for(int i=cases->first();cases->more(i);i=cases->next(i)) {
    emit_load_imm(T2,ct->class_name_map[dynamic_cast<branch_class*>(cases->nth(i))->type_decl]->get_tag(),s);

    //dbgStream<<"branch "<<i<<" tag= "<<ct->class_name_map[dynamic_cast<branch_class*>(cases->nth(i))->type_decl]->get_tag()<<endl;
 
    branch_labels.push_back(alloc_label());
    emit_beq(T1,T2,branch_labels[i],s);
  }
  ...
  emit_load_address(T2, CLASSPARENTNAMETAB, s);
  ...
  for (int i = cases->first(); cases->more(i); i = cases->next(i))
  {
    emit_label_def(branch_labels[i], s);

    // bind the branch var name to expr object that is already in the stack
    env.push_stack(dynamic_cast<branch_class*>(cases->nth(i))->name);

    dynamic_cast<branch_class*>(cases->nth(i))->expr->code(s);
